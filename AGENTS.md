...
9. 当前 Route A 已完成 S0-S3 阶段和 Phase A/B 平台能力升级。进入新的 `.slx` 结构或保真度改动前，先以 `04_Simulink物理网络模型/04_说明/RouteA_GasMixture_Derived/01_当前指导/RouteA_cEGR_PEMFC_模型裁决与资产处置_v01.md` 和 `04_Simulink物理网络模型/04_说明/RouteA_GasMixture_Derived/01_当前指导/RouteA_cEGR_PEMFC_收敛实施路线图_v01.md` 作为当前决策与路线真源，并以 `04_说明/RouteA_GasMixture_Derived/02_实施记录/01_当前分卷/` 作为变更证据；实施记录的触发和格式规则见"说明文件纪律"节以及 `02_实施记录/README.md`。RouteA_before 和 RouteA_v2 已归档至 `99_历史归档/2026-07-27_RouteA_before_and_v2_archived/`，不再作为活动资产。旧 `RouteA_cEGR_PEMFC_工程化建模规格_v01.md` 已移入 `99_历史归档/2026-07-25_RouteA_说明整理/`，不再作为当前规划真源。A6-A10、A11/A12 等阶段编号仅保留为历史实现证据或候选配置，不构成当前硬性推进顺序。
...
本节是项目级的说明文件维护规范，适用范围为 `04_Simulink物理网络模型/04_说明/RouteA_GasMixture_Derived/` 的全部说明内容。详细触发规则见该目录下 `02_实施记录/README.md`，本节只陈述纪律概要。

1. **文件分类**：说明文件分为"规划设计类"（`01_当前指导/`，覆盖式更新）和"实施记录类"（`02_实施记录/`，增量式更新）。`03_审计与研究/` 提供只读证据，`04_交接材料/` 仅用于跨客户端切换，均不参与以上分类。
2. **实施记录强制触发**：以下任一条件满足时，必须追加当前分卷（`02_实施记录/01_当前分卷/`）实施记录：(a) Phase X 完成并 git 提交时；(b) S 阶段收敛并 git 提交时；(c) 阻塞性问题解决（模型结构性修复、DAE IC 根因突破等）；(d) 新模型能力首次交付。
3. **记录不等于计划**：实施记录只记载实际发生过的工作、读回证据、验证结果和未解决风险。不得在运行前预写假设数值——验证结果必须从实际产物（CSV、workspace、logsout）回填。记录的"完成"以产物存在且验证通过为准，不以模型结构建成为准。
4. **阶段性任务完成后不写实施记录视为未完成**——这条是 agent 的工作纪律，不是用户的工作纪律。每次 git 提交或连续实施线收口时，agent 必须同步追加当前分卷实施记录，记录中每一个数值都应有对应的 CSV 列或 logsout 信号可追溯。
