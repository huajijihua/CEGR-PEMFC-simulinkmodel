...
| 简化台架 v1 | `01_自吸方案/03_台架测试_10kW_简化版/` | 当前主线 | L2 标定型 | 无空压机和加湿器 BOP，直接以台架入堆条件为边界 |
| COMSOL 机理 | `02_多物理场机理模型演示/` | 活跃 | L3 高保真 | 2D PEMFC + cEGR，用于局部机理研究和参数校核 |

## 当前主线快速入口

简化台架 v1 的主要脚本位于 `01_自吸方案/03_台架测试_10kW_简化版/02_脚本/`：
...
参数真源：`00_输入参数/标定参数/simplified_*.csv`。工况真源：`00_输入参数/实验数据/combined_noegr_cegr_fit_points.csv`。
...
6. `init_*_defaults.m` 中的 P 结构体是当前系统级默认参数真源，参数修改后要同步维护。
...
6. 任务结束前说明保留了哪些生成文件及用途；无保留价值的临时文件应清理或请求确认后清理。
