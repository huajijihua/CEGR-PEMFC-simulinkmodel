...
% Capture the disk-loaded model fingerprint before branch-specific runtime
% commands alter Model Workspace values.
topologyHashValue = topologyHash(model);
...
metadata.supplyBoundaryState = supplyBoundaryState(model);
metadata.modelVersion = modelVersion(modelFile);
metadata.topologyHash = topologyHashValue;
if isempty(fieldnames(parameterValues))
    metadata.parameterLayer = "platform_default";
else
    metadata.parameterLayer = "parameter_consistent_study";
end
metadata.externalCaseEnabled = false;
metadata.studyCommandBinding = "none";
...
commandBaseline = reshape(double(mw.getVariable( ...
    'routeA_command_profile_baseline')), 1, []);
if numel(commandBaseline) < 8 || any(~isfinite(commandBaseline(1:8)))
    error('RouteA:ParameterPreconditionCommandProfile', ...
        'The model workspace has no valid cathode outlet pressure baseline.');
end
condition = struct( ...
...
function state = supplyBoundaryState(model)
paths = routeA_block_paths(model);
state = struct();
state.schema = "RouteA_Supply_Boundary_v01";
state.model = string(model);
state.cathode = struct( ...
    'blockName', "Air Intake", ...
    'path', string(paths.airIntake), ...
    'species', ["N2", "O2", "H2", "H2O"], ...
    'compositionVariables', struct('o2', "env_yO2", ...
        'h2o', "env_yH20"));
state.anode = struct( ...
    'blockName', "Hydrogen Source", ...
    'path', string(paths.hydrogenSource), ...
    'species', ["N2", "O2", "H2", "H2O"], ...
    'compositionVariables', struct('h2', "tank_yH2"));
end

function hash = topologyHash(model)
checksum = Simulink.BlockDiagram.getChecksum(model);
hash = strjoin(string(dec2hex(uint32(checksum), 8)), '-');
end
