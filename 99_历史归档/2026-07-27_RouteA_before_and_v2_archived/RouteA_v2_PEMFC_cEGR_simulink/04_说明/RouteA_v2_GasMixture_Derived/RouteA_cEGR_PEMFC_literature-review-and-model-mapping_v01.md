...
日期：2026-07-24
状态：第一轮证据矩阵已完成，逐篇精读和 Phase 0.5 收口仍在进行；尚未据此修改 `.slx`。
范围：本地 8 篇直接 cEGR 论文、5 篇外部直接相关论文、1 篇前沿待复核论文和官方 Gas Mixture PEMFC 案例的交叉映射。
执行记录：[RouteA_v2_Execution_Record](../RouteA_v2_Execution_Record/README.md)，当前记录为 `R00_5_cegr_literature_mapping_20260724_v01.md`。
...
文献对 RouteA_v2 的作用分为四级，避免“读到现象 -> 直接加模块”的跳跃：

| 使用级别 | 可以决定什么 | 不可以决定什么 |
|---|---|---|
| `EVIDENCE` | 机制方向、变量关系、风险和研究问题 | 当前模型块连接或默认参数 |
| `INTERFACE` | 输入/输出、执行器、状态和 KPI 契约 | 未验证的设备细节 |
| `SCENARIO` | 最小验证工况、扰动和对照组 | 跨对象、跨功率等级的通用性能 |
| `PARAMETER_CANDIDATE` | 带适用范围的候选初值 | 未审计地进入 `platform_default` |

只有当一条证据同时具备对象、工况、变量定义、单位、来源和适用范围时，才允许从 `PARAMETER_CANDIDATE` 进入参数审计；通过参数审计和代表性运行后，才可能升级为 `PLATFORM_DEFAULT`。
...
第一轮矩阵只负责确定文献是否与 RouteA_v2 相关；Phase 0.5 收口前，每篇直接论文还必须完成以下字段。缺字段的论文可以保留为背景证据，但不能支撑结构修改或默认参数：

| 字段 | 记录要求 |
|---|---|
| 对象 | 单电池、单堆、系统或概念架构；规模、有效面积、单池数和功率等级 |
| 气路 | 新鲜空气、出口分流、阀/泵/阻力、混合点、排气和容腔 |
| 执行器 | 阀开度、泵速、背压、空气供给或其他实际控制对象 |
| 状态 | 压力、温度、气相水、液水/凝结、膜水代理、O2/N2/H2O 组成 |
| 口径 | 回流比、空气过量系数、O2 分压、RH、流量的分母、干湿基和方向 |
| 证据 | 实验、动态模型、稳态模型、概念分析或局部机理；是否有独立校核 |
| 适用范围 | 负荷、压力、温度、湿度、回流设备、时间尺度和对象限制 |
| 可迁移性 | 可迁移为机制、接口、工况、候选参数或仅作背景 |
| RouteA 绑定 | 对应变量、当前模块、最小验证 case 和失败分类 |

逐篇卡片可以写入本地精读笔记或阶段执行记录，但必须能从本文件的编号回查到原 PDF。不要只保留摘要句而丢失变量定义和适用范围。
...
RouteA_v2 不以“把所有影响项同时打开”为研究路线，而按问题树逐层开放：

```text
cEGR 气路闭合
|-- 阴极出口分流、执行器、混合、排气是否物理闭合？
|-- 回流组分、压力、温度和湿度是否来自出口网络？
`-- 质量/物种/压力响应是否可审计？
    |-- 低负荷：O2 分压与高电位限制/自增湿权衡
    |-- 额定附近：性能、RH、压降和控制跟踪
    |-- 高负荷：总流量、排水、氧供给和动态饥饿
    `-- 专项：主动泵寄生功耗、怠速和冷启动热/水状态
```

每个分支都必须单独声明执行器、状态、KPI、文献证据和停止条件。上一个分支通过只说明当前边界可用，不代表后一个分支的液水、热或设备动态已经存在。
...
| 研究影响/问题 | 物理变量 | 首要模型模块 | 最小验证 | 当前开放级别 |
|---|---|---|---|---|
| O2 稀释 | `yO2_ca_in`、`pO2_ca_in`、I/V | 阴极出口、cEGR 支路、入口混合、MEA | `cold_cegr_zero` -> `cold_cegr_small` | L1/L2 |
| 自增湿 | `RH_ca_in`、气相水、膜水代理 | cEGR 回流、加湿器、气体域、热端 | 低负荷稳态尾窗 | L2，需可观测水状态 |
| 高负荷排水 | 总流量、压差、液水/凝结、O2 | 背压/排气/水边界 | 高负荷动态 | 暂缓，直到液水边界明确 |
| 动态饥饿 | O2、压力、流量、电压下冲、限幅 | 空气供给、cEGR 设备、Stack_Core | step/ramp | 暂缓专项 |
| 寄生功耗 | 泵功率、阀压降、净功率 | Active cEGR 设备、Electrical/Results | 主动泵配置 | 暂缓专项 |
| 冷启动/怠速 | 热状态、液水/冻结假设、净功率 | Thermal、Water、cEGR 设备 | 独立热/水初态 | 概念研究，不进入默认门 |

该表是研究问题到验证的最小边界，不是声明这些能力当前都已经存在。
...
**出口条件：** 所有拟修改模块都有证据来源和验证目标；所有拟保留模块都有物理职责；所有暂缓能力都有明确原因；逐篇精读卡片的最低字段已完成或显式标记缺失；首个闭环用例和设备配置已固定；用户确认后才进入 `.slx` 结构修改。实际执行结论必须写入 `R00_5_cegr_literature_mapping_20260724_v01.md`，不能只修改本文档。
...
### 7.1 当前 Phase 0.5 收口状态

已完成：第一轮 8 篇本地直接论文的矩阵、补充文献的证据等级、cEGR 气路与影响项分层、变量口径初稿、当前 RouteA 资产保留/重构/暂缓方向。

尚未收口：

1. 逐篇精读卡片的对象规模、控制对象、口径和不可迁移参数需要继续回查原 PDF；
2. 首个闭环用例和被动/主动设备配置需要形成单一决策；
3. 当前两侧 `Source_Conditioner` 的每个端口需要与文献设备边界和当前物理网络逐项对应；
4. `platform_default` 候选值不能从论文直接复制，必须完成官方结构、量纲和适用范围审计；
5. 在上述项目完成前，不进入 Phase 1 的大规模结构修改。
...
任何报告中的文献驱动结论都必须同时写明：证据编号、对象和工况、变量口径、RouteA 模型变量、当前模块、适用范围、验证状态和结论强度。只有“趋势方向与论文一致”时，结论应写成趋势证据；没有独立实验或模型校核时，不写成产品性能承诺。

## 10. 未决问题

以下问题必须在进入 RouteA_v2 结构修改前收口，并在 Phase 0.5 执行记录中逐项判定：
