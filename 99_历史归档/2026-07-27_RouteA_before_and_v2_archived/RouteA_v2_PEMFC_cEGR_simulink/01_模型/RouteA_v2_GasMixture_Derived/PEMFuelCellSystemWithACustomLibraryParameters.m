...
routeA_cathode_source_conditioner_volume_L = 0.05; % [l] L2 cathode source-conditioning gas inventory
routeA_cathode_source_conditioner_nominal_flow_kg_s = 0.045; % [kg/s] Nominal mixed-source replenishment flow
...
routeA_anode_source_conditioner_volume_L = 0.05; % [l] L2 anode source-conditioning gas inventory
routeA_anode_source_conditioner_nominal_flow_kg_s = 0.002; % [kg/s] Nominal H2/N2 mixed-source replenishment flow
...
routeA_egr_target_input_mode_id = 1; % 0 scalar target, 1 workspace profile
...
%% Route A v10 unified runtime command profile
% Column 1 is model time. All other columns are continuous commands unless
% the runner encodes a discrete hold/switch transition explicitly. The saved
% operating point fixes physical inventory only; its baseline commands are
% held at logical t=0 and the study profile owns subsequent targets.
routeA_command_profile_schema = "RouteA_Command_Profile_v10";
...
routeA_command_profile_baseline = [ ...
    env_p, env_T, env_yO2, env_yH20, ...
    routeA_target_mdot_comp_inlet, routeA_target_oer, ...
    routeA_compressor_cmd_direct, routeA_target_p_ca_out_MPa, ...
    routeA_cathode_rh_setpoint, routeA_cathode_humidifier_gain, 0, ...
    tank_p, tank_T, tank_yH2, routeA_anode_inlet_pressure_MPa_abs, ...
    routeA_anode_rh_setpoint, routeA_anode_recirculation_base_command, ...
    routeA_anode_recirculation_current_gain_A_inv, routeA_anode_purge_enable, ...
    routeA_anode_purge_on_n2_mole_fraction, ...
    routeA_anode_purge_off_n2_mole_fraction, routeA_stack_temperature_set_C];
routeA_command_profile = [ ...
    0, routeA_command_profile_baseline; ...
    2501, routeA_command_profile_baseline];
