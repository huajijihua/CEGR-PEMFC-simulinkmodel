...
study.solver = struct('name', cfg.solver, ...
...
if ~builtin('all', arrayfun(@(item) abs(item.targetRatio - 0) < eps || ...
        abs(item.targetRatio - 0.30) < eps, caseOutputs))
    ledger.skipReason = "water_ledger_requires_zero_and_030_targets";
    return;
end
selected = caseOutputs(ismember([caseOutputs.targetRatio], [0, 0.30]));
...
if mode == "Current"
    ...
    waterCfg.currentTrackingWindow_s = [ ...
        waterCfg.researchStartTime_s, waterCfg.modelStopTime_s];
elseif mode == "Power"
    ...
    waterCfg.powerTrackingWindow_s = [ ...
        waterCfg.researchStartTime_s, waterCfg.modelStopTime_s];
else
    ...
    waterCfg.voltageTrackingWindow_s = [ ...
        waterCfg.researchStartTime_s, waterCfg.modelStopTime_s];
end
try
