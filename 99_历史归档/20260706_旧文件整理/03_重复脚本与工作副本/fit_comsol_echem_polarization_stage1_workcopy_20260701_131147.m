...
[opts, resumeInfo] = maybe_apply_checkpoint_resume(opts, baseParams);
...
checkpointState = init_checkpoint_state(opts, baseParams, resumeInfo);
runTimer = tic;
...
fprintf('Checkpoint enabled: %d\n', opts.checkpointEnabled);
...
checkpointState = update_checkpoint_best(checkpointState, baseParams, baseFit.objective, baseFit, baseVal, 0, 0, 'baseline');
save_checkpoint_if_enabled(opts, checkpointState);
...
if opts.evaluateOnly
    checkpointState.status = "evaluateOnly";
    checkpointState.lastUpdated = stamp_now();
    save_checkpoint_if_enabled(opts, checkpointState);
    return
end
...
checkpointState.finalResult = result;
checkpointState.bestParams = bestParams;
checkpointState.bestObjective = fBest;
checkpointState.bestFit = bestFit;
checkpointState.bestValidation = bestVal;
checkpointState.lastUpdated = stamp_now();
save_checkpoint_if_enabled(opts, checkpointState);
    ...
    function f = bounded_objective(z)
        ...
        checkpointState = append_checkpoint_history( ...
            checkpointState, evalCounter, trialParams, f, fitEv, valEv, evalElapsed_s, toc(runTimer));
        checkpointState = update_checkpoint_best( ...
            checkpointState, trialParams, f, fitEv, valEv, evalCounter, toc(runTimer), 'eval');
        save_checkpoint_if_enabled(opts, checkpointState);
        if opts.showEvalProgress
    ...
    function stop = report_optim_state(x, optimValues, state)
        ...
        switch string(state)
            case "init"
                ...
                checkpointState.status = "running";
                checkpointState.lastUpdated = stamp_now();
                save_checkpoint_if_enabled(opts, checkpointState);
            case "iter"
...
defaults.checkpointEnabled = true;
defaults.resumeFromCheckpoint = true;
defaults.checkpointFile = fullfile(root, 'outputs', 'optimization_checkpoints', ...
    'fit_comsol_echem_polarization_stage1_std1_checkpoint.mat');
defaults.checkpointSaveEveryEval = true;
...
function [opts, resumeInfo] = maybe_apply_checkpoint_resume(opts, baseParams)
resumeInfo = struct('applied', false, 'message', "", 'loadedCheckpoint', []);
if ~opts.checkpointEnabled || ~opts.resumeFromCheckpoint
    return
end
checkpointFile = string(opts.checkpointFile);
if strlength(checkpointFile) == 0 || ~isfile(checkpointFile)
    return
end

checkpoint = load_checkpoint_file(checkpointFile);
if isempty(checkpoint)
    resumeInfo.message = "checkpoint-load-failed";
    return
end
if ~isfield(checkpoint, 'bestParams') || isempty(checkpoint.bestParams)
    resumeInfo.message = "checkpoint-missing-bestparams";
    return
end
if ~isfield(checkpoint, 'fitCaseIdx') || ~isequal(checkpoint.fitCaseIdx(:), opts.fitCaseIdx(:))
    resumeInfo.message = "checkpoint-fitcases-mismatch";
    return
end
if ~isfield(checkpoint, 'validationCaseIdx') || ~isequal(checkpoint.validationCaseIdx(:), opts.validationCaseIdx(:))
    resumeInfo.message = "checkpoint-validationcases-mismatch";
    return
end
if ~isfield(checkpoint, 'studyTag') || ~strcmp(string(checkpoint.studyTag), string(opts.studyTag))
    resumeInfo.message = "checkpoint-studytag-mismatch";
    return
end
if ~isfield(checkpoint, 'activeParams') || ~isequal(string(checkpoint.activeParams(:)), string(opts.activeParams(:)))
    resumeInfo.message = "checkpoint-activeparams-mismatch";
    return
end

opts.initialParams = merge_full_echem_params(baseParams, checkpoint.bestParams);
resumeInfo.applied = true;
resumeInfo.message = "checkpoint-resume-applied";
resumeInfo.loadedCheckpoint = checkpoint;
fprintf('Resume checkpoint loaded from %s\n', opts.checkpointFile);
if isfield(checkpoint, 'bestObjective') && ~isempty(checkpoint.bestObjective)
    fprintf('Checkpoint best objective = %.6g\n', checkpoint.bestObjective);
end
end

function checkpoint = load_checkpoint_file(checkpointFile)
checkpoint = [];
try
    ...
    warning('fit_comsol_echem_polarization_stage1:CheckpointLoadFailed', ...
        'Failed to load checkpoint file %s: %s', checkpointFile, ME.message);
end
end
...
function checkpointState = init_checkpoint_state(opts, baseParams, resumeInfo)
checkpointState = struct();
checkpointState.version = 1;
checkpointState.script = mfilename('fullpath');
checkpointState.studyTag = opts.studyTag;
checkpointState.fitCaseIdx = opts.fitCaseIdx(:);
checkpointState.validationCaseIdx = opts.validationCaseIdx(:);
checkpointState.activeParams = opts.activeParams(:);
checkpointState.baseParams = baseParams;
checkpointState.bestParams = [];
checkpointState.bestObjective = inf;
checkpointState.bestFit = [];
checkpointState.bestValidation = [];
checkpointState.bestEvalCounter = NaN;
checkpointState.history = struct([]);
checkpointState.status = "initialized";
checkpointState.createdAt = stamp_now();
checkpointState.lastUpdated = checkpointState.createdAt;
checkpointState.resumeApplied = resumeInfo.applied;
checkpointState.resumeMessage = resumeInfo.message;
checkpointState.resumeSource = string(opts.checkpointFile);
checkpointState.lastIter = struct([]);
checkpointState.finalResult = [];
if resumeInfo.applied && isstruct(resumeInfo.loadedCheckpoint)
    if isfield(resumeInfo.loadedCheckpoint, 'bestParams')
        checkpointState.bestParams = resumeInfo.loadedCheckpoint.bestParams;
    end
    if isfield(resumeInfo.loadedCheckpoint, 'bestObjective')
        checkpointState.bestObjective = resumeInfo.loadedCheckpoint.bestObjective;
    end
    if isfield(resumeInfo.loadedCheckpoint, 'bestFit')
        checkpointState.bestFit = resumeInfo.loadedCheckpoint.bestFit;
    end
    if isfield(resumeInfo.loadedCheckpoint, 'bestValidation')
        checkpointState.bestValidation = resumeInfo.loadedCheckpoint.bestValidation;
    end
    if isfield(resumeInfo.loadedCheckpoint, 'bestEvalCounter')
        checkpointState.bestEvalCounter = resumeInfo.loadedCheckpoint.bestEvalCounter;
    end
    if isfield(resumeInfo.loadedCheckpoint, 'history')
        checkpointState.history = resumeInfo.loadedCheckpoint.history;
    end
end
end

function checkpointState = append_checkpoint_history(checkpointState, evalCounter, trialParams, objectiveValue, fitEv, valEv, evalElapsed_s, wallTime_s)
entry = struct();
entry.evalCounter = evalCounter;
entry.params = trialParams;
entry.objective = objectiveValue;
entry.fitObjective = fitEv.objective;
entry.fitRmse_V = rmse(fitEv.err_V);
if isempty(valEv)
    entry.validationObjective = NaN;
    entry.validationRmse_V = NaN;
else
    entry.validationObjective = valEv.objective;
    entry.validationRmse_V = rmse(valEv.err_V);
end
entry.evalElapsed_s = evalElapsed_s;
entry.wallTime_s = wallTime_s;
entry.timestamp = stamp_now();
checkpointState.history = [checkpointState.history; entry];
checkpointState.lastUpdated = entry.timestamp;
end

function checkpointState = update_checkpoint_best(checkpointState, params, objectiveValue, fitEv, valEv, evalCounter, wallTime_s, reason)
if ~isfinite(objectiveValue) || objectiveValue >= checkpointState.bestObjective
    return
end
checkpointState.bestParams = params;
checkpointState.bestObjective = objectiveValue;
checkpointState.bestFit = fitEv;
checkpointState.bestValidation = valEv;
checkpointState.bestEvalCounter = evalCounter;
checkpointState.bestReason = string(reason);
checkpointState.bestWallTime_s = wallTime_s;
checkpointState.lastUpdated = stamp_now();
end

function save_checkpoint_if_enabled(opts, checkpointState)
if ~opts.checkpointEnabled
    return
end
checkpoint = checkpointState; %#ok<NASGU>
checkpointDir = fileparts(opts.checkpointFile);
try
    if ~isempty(checkpointDir) && ~isfolder(checkpointDir)
        mkdir(checkpointDir);
    end
    save(opts.checkpointFile, 'checkpoint', '-mat');
catch ME
    warning('fit_comsol_echem_polarization_stage1:CheckpointSaveFailed', ...
        'Failed to save checkpoint file %s: %s', opts.checkpointFile, ME.message);
end
end
