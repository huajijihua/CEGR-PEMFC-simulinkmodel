function metadata = routeA_generate_platform_default_voltage_initial_state(sourceInitialStateFile)
...
preconditionCfg = struct( ...
    'loadInputType', "Voltage", ...
    'targetVoltage_V', sourceVoltage_V, ...
    'maxStep_s', 5);
...
if ~audit.periodic.passed
    error('RouteA:VoltageInitialStatePeriodic', ...
        'The Voltage candidate did not pass periodic verification.');
end
if abs(metadata.cegrValveMaxArea_m2 - source.cegrValveMaxArea_m2) > ...
        1e-12 * max(1, abs(source.cegrValveMaxArea_m2))
    error('RouteA:VoltageInitialStateParameterMismatch', ...
...
assignin('base', 'routeA_platform_default_voltage_candidate_metadata', ...
    routeA_initial_metadata);
fprintf(['Saved Route A mode-1 Voltage initial-state candidate: %s ', ...
    '(V_source=%.4g V)\n'], candidateFile, sourceVoltage_V);
