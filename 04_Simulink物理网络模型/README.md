...
当前处于 Stage 1 已闭合、后续专题按规格逐项冻结后实施的工程化建模阶段。新的 `.slx`、参数、控制或 runner 改动必须先明确研究边界、保持不变的假设和验收门，并在模型读回与最小必要仿真验证后进入里程碑提交。
